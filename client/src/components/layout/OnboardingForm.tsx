...
export default function OnboardingForm({ onSubmit, initialIdea = "" }: OnboardingFormProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] text-center max-w-lg mx-auto space-y-8 animate-in fade-in duration-500">
        <div className="relative">
          <div className="w-24 h-24 rounded-full border-4 border-blue-500/10 border-t-blue-600 animate-spin flex items-center justify-center shadow-lg shadow-blue-500/10" />
        ...
        {/* Visual progress bar */}
        <div className="w-full bg-slate-50 h-1.5 rounded-full overflow-hidden border border-slate-200">
          <div 
  ...
  return (
    <Card className="max-w-xl mx-auto bg-slate-50/40 border-slate-200 backdrop-blur-2xl shadow-2xl p-8 rounded-2xl relative overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-32 bg-blue-600/10 blur-[80px] rounded-full -z-10" />
      
      <CardHeader className="px-0 pt-0 pb-6 border-b border-slate-200">
        <div className="flex items-center gap-2 mb-2">
          <Rocket className="w-6 h-6 text-blue-500" />
          <span className="font-extrabold text-slate-900 tracking-tight text-xl">Validate Your Startup Idea</span>
        </div>
          ...
          {STEPS.map((s, idx) => (
            <div key={idx} className="flex-1 space-y-1.5">
              ...
              <span className={`text-[10px] font-bold block transition-colors ${
                idx === step ? "text-blue-600" : "text-slate-600"
              }`}>
                Step {idx + 1}
              </span>
            </div>
      ...
      <CardContent className="px-0 pt-8 space-y-6">
        {step === 0 && (
          <div className="space-y-4 animate-in fade-in duration-300">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Startup Name</label>
              <Input
                value={form.name}
                onChange={(e) => setForm({ ...form, name: e.target.value })}
                placeholder="e.g. EcoStream AI or FitPath"
                className="bg-white/60 border-slate-200 text-slate-900 rounded-xl h-11 focus-visible:ring-blue-600 placeholder:text-slate-600"
              />
            </div>
            <div className="space-y-1.5">
        ...
        {step === 1 && (
          <div className="space-y-4 animate-in fade-in duration-300">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Startup Concept & Idea</label>
              <textarea
                value={form.idea}
                onChange={(e) => setForm({ ...form, idea: e.target.value })}
                placeholder="Describe what your startup does, who it is for, and how it solves their problem in detail..."
                className="w-full min-h-[120px] bg-white/60 border border-slate-200 text-slate-900 rounded-xl p-3 focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm placeholder:text-slate-600 resize-none transition-all"
              />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Target Market</label>
              <select
                value={form.country}
                onChange={(e) => setForm({ ...form, country: e.target.value })}
                className="w-full bg-white/60 border border-slate-200 text-slate-900 rounded-xl h-11 px-3 focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm transition-all"
              >
        ...
        {step === 2 && (
          <div className="space-y-4 animate-in fade-in duration-300">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Estimated Launch Budget</label>
              <select
                value={form.budget}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Revenue Model</label>
              <select
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-200">
          {step > 0 && (
            <Button
              variant="outline"
              onClick={handleBack}
              className="border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100 rounded-xl h-11 flex-1 font-bold text-sm"
            >
              Back
            </Button>
          )}
          <Button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl h-11 flex-[2] font-bold text-sm gap-2"
          >
