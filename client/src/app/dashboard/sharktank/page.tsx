...
export default function RealityCheckModule() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">Investor Readiness & Reality Check</h1>
        <p className="text-slate-400">Evaluate investor readiness scores and enter the brutal gauntlet check.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        {/* Score Card */}
        <Card className="bg-slate-900/50 border-white/5 flex flex-col items-center justify-center p-8 text-center">
          <Award className="w-12 h-12 text-indigo-400 mb-4 animate-bounce" />
          <h2 className="text-lg font-bold text-slate-300">Attractiveness Score</h2>
          ...
          <Progress value={investorScore} className="h-2 w-full bg-white/5" indicatorClassName="bg-indigo-500" />
        ...
        <Card className="lg:col-span-2 bg-slate-900/50 border-white/5">
          ...
          <CardContent className="space-y-4">
            {metrics.map((metric, i) => (
              <div key={i} className="flex flex-col md:flex-row md:items-center justify-between pb-3 border-b border-white/5 last:border-0 last:pb-0">
                <span className="text-sm font-bold text-white mb-1 md:mb-0">{metric.label}</span>
                <span className="text-xs text-slate-400 text-right max-w-md italic">"{metric.desc}"</span>
              </div>
            ))}
          </CardContent>
        </Card>
      ...
      <Card className="bg-slate-900/50 border-white/5 mb-8">
        <CardHeader>
      ...
      <div className="max-w-4xl mx-auto border-t border-white/5 pt-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-400">Brutally honest feedback on your startup idea. No sugar-coating.</p>
          </div>
        ...
        {!isActive ? (
          <Card className="bg-slate-900 border-white/5 py-12 text-center border-dashed border-2">
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-bold text-white">Enter the Gauntlet</h3>
          ...
          <div className="grid grid-cols-1 gap-6 animate-in fade-in zoom-in duration-500">
            {threats.map((fact: any, i: number) => {
              ...
              return (
                <Card key={i} className="bg-slate-900/50 border-white/5 group hover:border-rose-500/30 transition-all">
                  <CardContent className="p-6 flex gap-6">
                    <div className="shrink-0 w-16 h-16 rounded-2xl bg-slate-800 border border-white/5 flex items-center justify-center group-hover:bg-rose-500/10 group-hover:border-rose-500/20 transition-all">
                      <IconComponent className="w-8 h-8 text-slate-400 group-hover:text-rose-500" />
                    </div>
            ...
            <div className="flex justify-center mt-8">
               <Button variant="ghost" className="text-slate-500 hover:text-white" onClick={() => setIsActive(false)}>
                Reset Reality Check
              </Button>
            </div>
          </div>
