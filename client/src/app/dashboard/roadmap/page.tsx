...
export default function RoadmapPage() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Go-To-Market & Roadmap</h1>
        <p className="text-slate-500">Step-by-step user acquisition plan and interactive launch timeline.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
              ...
              <div className="pt-4 border-t border-slate-200">
                ...
                <div className="flex flex-wrap gap-2">
                  {gtm?.marketingChannels?.map((ch: string, i: number) => (
                    <span key={i} className="text-[10px] font-bold text-blue-600 bg-blue-600/10 border border-blue-500/20 px-2 py-1 rounded">
                      {ch}
                    </span>
                  ))}
                </div>
        ...
        <Card className="lg:col-span-2 bg-slate-50/50 border-slate-200">
          ...
          <CardContent className="relative pl-6 border-l border-slate-200 space-y-6">
            {timelineSteps.map((step, i) => (
              <div key={i} className="relative group">
                {/* Timeline node */}
                <div className="absolute -left-[30px] top-1.5 w-4.5 h-4.5 rounded-full bg-white border border-blue-500 flex items-center justify-center text-[8px] font-black text-blue-600 shadow-md">
                  {i + 1}
                </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              ...
              <div 
                ...
                className={cn(
                  ...
                  activePhase === i 
                    ...
                    : "bg-slate-50/30 text-slate-500 border-slate-200 hover:border-slate-200"
                )}
              >
                <div>
          ...
          {roadmap[activePhase] && (
            <Card className="lg:col-span-3 bg-slate-50/50 border-blue-500/20">
              <CardHeader className="border-b border-slate-200">
                <div className="flex justify-between items-center">
                  <CardTitle className="text-xl font-extrabold text-slate-900">
                    {roadmap[activePhase].title}
                  </CardTitle>
                  <span className="text-xs font-bold text-blue-600 uppercase tracking-widest bg-blue-600/10 px-2 py-0.5 rounded border border-blue-500/20">
              ...
              <CardContent className="pt-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {roadmap[activePhase].tasks?.map((task: string, i: number) => (
                    <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-slate-50 border border-slate-200 hover:border-blue-500/20 transition-all">
                      <CheckCircle2 className="w-4.5 h-4.5 text-blue-500 shrink-0 mt-0.5" />
                      <span className="text-xs text-slate-700 leading-relaxed">{task}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
